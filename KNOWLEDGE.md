...
`buildWsAdapter().startDialog` は `({ script, effects }) => Promise` を返し、ダイアログが閉じる（onExit→resolve）まで解決しない。これにより `_runEvent` の await が保たれ、同一trigger 内の複数イベントが**直列表示**される。

**会話ごとに ADVScene を強制再マウントする仕様（2026-06-02）**: 各 `startDialog`/theater 起動時に `dialogSeqRef.current` をインクリメントした `dialogId` を付与し、`<ADVScene key={sceneParams.dialogId ?? 'adv'}>` で描画。key 変化により React が unmount→remount し、`idx`/`finishedRef`/`history` を確実にリセットする。同一 scene='adv' のまま逐次2件目が来てもフリーズしない。
...
### 終盤シナリオ復元（埼玉TL / ボカロTL）（2026-06-05）
- `PROMPT_event_renewal` が削除した `ch04_saitama_tl/`（5件）・`ch05_vocalo/`（4件）を `PROMPT_scenario_endgame_restore` で復元（コンフリクト解消は scenario_phase1 採用）。
- 復元9件は全ダミーテキスト（`【ダミー】`）・`player_turn` trigger・コード変更ゼロ。`_index.json` events は 22→**31件**。
- 終盤チェーン: 春日部制圧→`flag_saitama_conquered_by_player`＋東京宣戦 / T55未達は `baseTransfer`(green→東京)吸収 / 京都制圧→AHS撃破 / T80吸収→T85ボカロ宣戦 / 北九州制圧→`flag_vocalo_conquered`→`checkVictoryCondition` で `victory`（GameContext.jsx:736 結線済）。
- **残**: ダミーテキスト差し替え・charJoin（ウナしゅお/ずん子いたこ解禁の実合流処理。現状フラグのみ）。
- **別件（スコープ外）**: `ch02_saitama/ev_saitama_chain_3` の `trigger: "turn_start"` は未接続trigger疑い→chain停止で `ev_saitama_chain_4` の `legionForceAttack` 不発の可能性。要調査。
