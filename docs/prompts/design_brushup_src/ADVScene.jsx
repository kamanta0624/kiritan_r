...
// charKey（characters.json の id, 例 char_004）から正準立ち絵パスを解決。
// 規約パス /characters/portraits/<charKey>.png 一本。見つからなければ null →
// プレースホルダへ倒す（tokens.js のハードコード立ち絵にはフォールバックしない）。
...
// ── Persona-style cutin ─────────────────────────────────
function PersonaCutin({ entry, char, charKey, color, onAdvance }) {
  const src = getPortrait(charKey);
  const [imgSrc, setImgSrc] = useState(src);
  useEffect(() => { setImgSrc(src); }, [src]);
  const handleError = () => setImgSrc(null);
  ...
  script.forEach((step, origIdx) => {
    ...
    switch (step.type) {
      ...
      case 'dialog': // 既に内部形式（DEMO_SCENARIO 等）
        scenario.push({ ...step, speaker: resolveCharId(step.speaker), charKey: step.speaker });
        break;
      case 'choice':
        scenario.push({
...
function buildCast(script = []) {
  ...
  const add = (characterId, position) => {
    ...
    if (cid && position && getChar(cid) && !castMap.has(position))
      castMap.set(position, { id: cid, charKey: characterId });
  };
  script.forEach(step => {
  ...
  return Array.from(castMap.entries()).map(([pos, { id, charKey }]) => ({ id, pos, charKey }));
}
