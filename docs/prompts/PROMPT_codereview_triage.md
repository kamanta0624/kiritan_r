...
- コード根拠: `src/App.jsx:233-242` に早期returnが実在。
  ...
  ```
- 本番QA手順も `http://localhost:5174/?qa=battlefull`。QAシーンは生存コード
- 対応: QAシーン3ファイル削除禁止。`PROMPT_delete_qa_scenes.md` 破棄
...
| ID | ファイル | 内容 |
|----|---------|------|
| BUG-09 | BattleScene/App | 野戦選択時の `battleCapacity` がEngineに未伝達。野戦完全無効（深刻度高） |
...
| # | プロンプト案 | 含む項目 | 範囲 | 責任 |
|---|------------|---------|------|------|
| 1 | PROMPT_battlecap_fix | BUG-09 | BattleScene/App | 野戦battleCapacityをEngineへ伝達 |
